  ...
  useEffect(() => {
    ...
    fetch(url, { signal: controller.signal })
      ...
      .catch((e) => {
        ...
        if (active) {
          console.warn(
            "WeatherWidget API failed or timed out:",
            e.message,
            "- Falling back to dummy weather.",
          );
          loadDummyWeather();
        }
      });
    ...
    <div>
      {isDemo && (
        <p className="mb-3 text-[11px] uppercase tracking-wider text-muted-foreground">
          Sample forecast — live weather unavailable
        </p>
      )}
      ...
      <div className="mt-6 grid grid-cols-7 gap-2">
        {data.days.map((d) => {
          ...
          return (
            <div
              key={d.date}
              className="rounded-lg bg-background/60 border border-border/60 p-2 text-center"
            >
