  ...
  for (const q of queries) {
    try {
      ...
      if (summaryData.extract) {
        return { extract: summaryData.extract, title: pageTitle };
      }
    } catch (e) {
      // Continue searching
    }
...
export const describePlace = createServerFn({ method: "POST" })
  .inputValidator((input: unknown) => Input.parse(input))
  .handler(async ({ data }): Promise<PlaceInfo> => {
    const locale = [data.name, data.admin, data.country].filter(Boolean).join(", ");

    // Fetch Wikipedia summary first to ground the AI and save credits/reasoning
...
function parseAIResult(content: string): PlaceInfo {
  try {
    return JSON.parse(content) as PlaceInfo;
  } catch {
    const cleaned = content.replace(/```json\s*|\s*```/g, "").trim();
    return JSON.parse(cleaned) as PlaceInfo;
  }
}

function getFallbackPlaceInfo(name: string, country?: string, wikiExtract?: string): PlaceInfo {
  ...
  if (isBirla) {
    return {
      ...
      timeline: [
        ...
        { year: "Today", event: "Remains a major cultural and architectural symbol of Hyderabad." },
      ],
      hotels: [
        {
      ...
      tips: [
        "Cameras and mobile phones are strictly prohibited inside the temple premises.",
        "Remove footwear before climbing the temple steps; free storage is available at the base.",
        "Visit during early morning or sunset for a peaceful experience and pleasant weather.",
      ],
      openingHours: "7:00 AM - 12:00 PM, 3:00 PM - 9:00 PM daily",
      entryFees: "Free Admission",
      safetyTips: [
        "Keep your personal belongings secure, especially in crowded queue lines during festivals.",
        "Dress modestly in accordance with temple customs.",
        "Beware of unauthorized guides or sellers at the entrance.",
      ],
      photographyTips: [
        "Photography is strictly banned inside. You can take photos of the outer hillsides from the main road.",
        "The view of the city and lake from the temple entrance stairs at sunset is stunning.",
        "Use a wide-angle lens from the base of the hill to capture the entire marble structure.",
      ],
      restaurants: [
        {
      ...
      experiences: [
        "Participate in the evening Aarti (prayer service) for a deeply spiritual atmosphere.",
        "Stroll around the hilltop platform to enjoy a 360-degree panoramic view of Hyderabad and Secunderabad.",
        "Browse the religious book stall and souvenir shop at the base of the temple.",
      ],
      transportation:
        "Easily accessible by auto-rickshaw, local taxis, or metro (Assembly Metro Station is 1.5 km away). Local buses drop off at the Lakdikapul stop.",
      travelInsights:
        "Birla Mandir has no traditional bells (ghantas) to maintain a meditative, silent environment. The presiding deity is a replica of the Lord Venkateswara statue at Tirupati, carved from a single piece of black granite.",
      reviews: [
        {
          author: "Rahul Sharma",
          rating: 5,
  ...
  if (wikiExtract) {
    return {
      ...
      timeline: [
        { year: "Historical", event: "Noted in regional records and cultural studies." },
        { year: "Today", event: "Maintained as an active heritage landmark." },
      ],
      hotels: [
        {
      ...
      tips: [
        ...
        "Hire a certified local guide for deeper context.",
      ],
      ...
      photographyTips: [
        "Take photos during early morning for golden light.",
        "No flash inside the closed areas.",
      ],
      restaurants: [
        {
      ...
      experiences: [
        "Explore the main structure and surrounding grounds.",
        "Take scenic photographs from surrounding viewpoints.",
      ],
  ...
  return {
    ...
    significance:
      "Stands as a vital cultural site preserving historical heritage and local art forms.",
    architecture:
      "Exhibits classical regional styles built with local materials and intricate stonework.",
    bestTime: "October – March (during cooler seasons)",
    timeline: [
      { year: "1800s", event: "Establishment and early development." },
      { year: "1900s", event: "Recognized as a protected heritage landmark." },
      { year: "Today", event: "Visited by thousands of global travelers annually." },
    ],
    hotels: [
      {
        name: "Grand Plaza Hotel",
      ...
      { name: "Heritage Inn", rating: 4.2, pricePerNight: 60, distanceKm: 2.2, tag: "Authentic" },
    ],
    nearby: ["City Palace", "Local Museum", "Central Square", "Old Market"],
    tips: [
      "Hire a local guide to get detailed historical context.",
      "Wear comfortable walking shoes as there is plenty to explore.",
    ...
    safetyTips: [
      ...
      "Ensure you keep track of your belongings.",
      "Stay in well-lit public paths.",
    ],
    photographyTips: [
      "Golden hour yields the best color tones on stone structures.",
      "No flash inside the museum area.",
      "Capture the grand entry arches from a distance.",
    ],
    restaurants: [
      {
        name: "Local Flavors Diner",
    ...
    experiences: [
      "Join a local guided walking tour.",
      "Taste local street food specialties nearby.",
      "Attend the evening cultural music program.",
    ],
    transportation: "Conveniently connected by regional buses and local taxi services.",
    travelInsights:
      "An essential stop for anyone wanting to capture the historical essence of the region.",
    reviews: [
      {
        author: "Jane Doe",
        rating: 5,
