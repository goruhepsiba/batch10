...
export interface PlaceInfo {
  ...
  hotels: { name: string; rating: number; pricePerNight: number; distanceKm: number; tag: string }[];
  nearby: string[];
  tips: string[];
  openingHours: string;
  entryFees: string;
  ...
  restaurants: { name: string; cuisine: string; rating: number; priceRange: string; distanceKm: number }[];
  experiences: string[];
...
export const describePlace = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<PlaceInfo> => {
    ...
    const user = `Write a comprehensive heritage-travel profile for: ${locale}.
    ...
    return getFallbackPlaceInfo(data.name);
  });
...
function getFallbackPlaceInfo(name: string): PlaceInfo {
  ...
  if (isBirla) {
    return {
      ...
      timeline: [
        ...
        { year: "1976", event: "Inaugurated and dedicated to the public by Swami Ranganathananda." },
      ...
      hotels: [
        ...
        { name: "Taj Mahal Hotel Abids", rating: 4.2, pricePerNight: 55, distanceKm: 1.5, tag: "Heritage Vegetarian Hotel" },
        { name: "Mercure Hyderabad KCP", rating: 4.4, pricePerNight: 75, distanceKm: 2.8, tag: "Business Premium" }
      ],
      ...
      restaurants: [
        { name: "Bikanervala Hyderabad", cuisine: "North Indian & Sweets", rating: 4.3, priceRange: "$$", distanceKm: 0.8 },
        { name: "Kamath Vegetarian", cuisine: "South Indian Thali", rating: 4.1, priceRange: "$", distanceKm: 1.2 },
        { name: "Taj Mahal Restaurant", cuisine: "Pure Veg Udupi", rating: 4.4, priceRange: "$$", distanceKm: 1.5 }
      ],
      experiences: [
      ...
      reviews: [
        { author: "Rahul Sharma", rating: 5, text: "Absolutely peaceful place. The white marble looks stunning at night when illuminated. Highly recommended!", date: "2026-05-15" },
        { author: "Emma Watson", rating: 4, text: "Beautiful views of the city. No phones allowed inside, so prepare to just disconnect and enjoy the serenity.", date: "2026-05-20" },
        { author: "Suresh Kumar", rating: 5, text: "Very well maintained temple. Best place to spend a quiet evening with family.", date: "2026-06-01" }
      ]
    };
  }
  ...
  return {
    ...
    hotels: [
      { name: "Grand Plaza Hotel", rating: 4.4, pricePerNight: 80, distanceKm: 1.5, tag: "Comfort" },
    ...
    restaurants: [
      { name: "Local Flavors Diner", cuisine: "Traditional Cuisine", rating: 4.3, priceRange: "$$", distanceKm: 0.5 }
    ],
    experiences: [
    ...
    reviews: [
      { author: "Jane Doe", rating: 5, text: "A truly historical masterpiece. Absolutely loved the atmosphere!", date: "2026-05-10" }
    ]
  };
}
