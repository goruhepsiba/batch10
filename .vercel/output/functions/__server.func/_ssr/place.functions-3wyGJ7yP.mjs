...
import { c as createSsrRpc, u as useAuth } from "./router-BAKaFseM.mjs";
import { c as createServerFn } from "./server-rHDi7l_h.mjs";
import { H as Heart, R as Rotate3d, x as Map, N as Maximize2, O as ExternalLink } from "../_libs/lucide-react.mjs";
...
function FavoriteButton({ kind, refId, name, country, lat, lng, size = "md" }) {
  ...
  reactExports.useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const { data } = await supabase.from("favorites").select("id").eq("user_id", user.id).eq("kind", kind).eq("ref", refId).maybeSingle();
