...
Generate exactly ${data.days} days. All costs MUST be in Indian Rupees (₹). Keep all activity descriptions short but high quality and informative (15-20 words max per time slot).`;
  ...
  const geminiKey = process.env.GEMINI_API_KEY?.startsWith("AIzaSy") ? process.env.GEMINI_API_KEY : process.env.AI_API_KEY?.startsWith("AIzaSy") ? process.env.AI_API_KEY : void 0;
  if (geminiKey) {
    try {
...
function getFallbackItinerary(destination, days, budget) {
  ...
  const costPerDay = budget === "budget" ? 2e3 : budget === "mid" ? 5e3 : 12e3;
  if (isHyderabad) {
    return {
      ...
      estimatedCostINR: {
        low: costPerDay * days * 0.8,
        high: costPerDay * days * 1.3
      },
      ...
      tips: ["Travel via local auto-rickshaw — negotiate ₹30-80 per ride.", "Buy pearls only from certified outlets in Laad Bazaar.", "Try Irani Chai at Nimrah Café near Charminar for ₹25."]
    };
  }
  return {
    destination,
    ...
    tips: ["Carry cash for entry fees at heritage monuments.", "Stay hydrated and wear comfortable walking shoes.", "Start early morning to beat the crowds at popular sites."]
  };
}
export {
